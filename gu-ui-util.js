...
var gu = {
  ...
  slideChange: function(btn, element, ul, li, span, brother, besideCall) {
    gu.onClick(btn, function($this) {
      ...
      ul.stop().animate({'margin-left' : ((li.width()) * (myIndex)) * -1}, 0, function() {gu.slideCountText(ul, li, element.find('.gu_slide_page_text > span'))});
  ...
  resultHeight: function(li, margin) {var value = 0; for (var i = 0; i < li.length; i++) {value = value + (li.eq(i).outerHeight()) + margin;} return value;},
  ...
  open: function(url, width, height) {window.open(url, '_blank', "width=" + width + ", height=" + height)},
  // 20180710
  hoverChange: function(btn, element, ul, li, span, brother, hoverEventCall) {
    gu.onHover(btn, function($this) {
      var myIndex = $this.parent('li').index(); gu.classCall($this, brother);
      ul.stop().animate({'margin-left' : ((li.width()) * (myIndex)) * -1}, 0, function() {gu.slideCountText(ul, li, element.find('.gu_slide_page_text > span'))});
      hoverEventCall && hoverEventCall($this);
    }, function() {});
  }
};
...
//20180723 scroll top cookie (s)
var srv = parseInt($.cookie('scrValue'));
gu.timeCallback(500, function() {
  $('html, body').stop().animate({'scrollTop' : srv}, 0);
});
